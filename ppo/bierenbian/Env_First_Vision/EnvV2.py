...
class Env_version2(gym.Env):
    def __init__(self):
        ...
        self.observation_space = Box(
            low=np.array([-20, -20, 0.5,180,180,180], dtype=np.float32),
            high=np.array([20, 20, 0.5,-180,-180,-180], dtype=np.float32))
    ...
    def get_constraint(self):
        X_state_constraint = [lambda x: linalg.norm(x - np.array([4,0,0.5])) < 4,
                              lambda x: linalg.norm(x - np.array([4,0,0.5])) > 6]
    ...
    def reset(self):
        ...
        return Ball_ob[:6],dis_reach_goal
