...
class Env_simple(gym.Env):
    def __init__(self):
        ...
        self.observation_space = Box(
            low=np.array([-4, 0, 0,180,180,180], dtype=np.float32),
            high=np.array([4, 0, 0,-180,-180,-180], dtype=np.float32))
    ...
    #     X_state_constraint = [lambda x: np.abs(x[0]) < 8]
    ...
    def get_constraint(self):
        X_state_constraint = [lambda x: abs(x[0])-7.5]
        return X_state_constraint
