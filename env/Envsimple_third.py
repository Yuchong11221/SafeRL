...
class Env_simple(gym.Env):
    def __init__(self,
        ...
        self.action_space = Box(low=-8, high=8, shape=(1,), dtype=np.float32)
    ...
    def compute_done(self):
        dist = np.linalg.norm(self.Ball.get_observation()[0] - self.target_position)
        if dist < 0.5:
            done = True
    ...
    def get_constraint_value(self,pos):
        ...
        left_constraints = -7.5 - pos[0]
        right_constraint = pos[0]-7.5
        return np.array([left_constraints, right_constraint])

    def constraint_activate(self,pos):
        c_now = self.get_constraint_value(pos)
        c_bar = 0
        if np.any(c_now>0):
            c_bar = 1
        else:
    ...
    def reward(self):
        if self.outside_boundary():
            ...
            reward_outside = -1
        # L2 norm
        dist = np.linalg.norm(self.Ball.get_position()[0] - self.target_position)
    ...
    def step(self, action):
        ...
        observation = Ball_ob
        return observation[[0,3]], reward, done, self.target_position[[0,1]]

    def reset(self,random_set = False):
        # Reset environement
        p.setGravity(0, 0, -10)
        self.Ball.reset(random_set)
